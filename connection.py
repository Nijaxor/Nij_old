import socket, ssl, threading, printer, copy, os
def Connection(**ircStuff):
	...
	try:
		...
		if len(perms) > 0:
			for line in perms:
				...
				if len(linesplit) > 1:
					permissions[linesplit[0].lower()] = [linesplit[1:], linesplit[0]]
	...
	while 1:
		...
		threading.Thread(target=ircStuff["processlineModule"].processLine, args=(line,), kwargs=ircStuff).start()
